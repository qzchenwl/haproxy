...
static void init_new_proxy(struct proxy *p)
{
    ...
    // TODO cwl init hashtable
    p->switching_hashtbl = hashtbl_create(HASH_SIZE, NULL);
    // TODO cwl end init hashtable
}
...
int cfg_parse_listen(const char *file, int linenum, char **args, int kwm)
{
    ...
    if (!strcmp(args[0], "bind")) {  /* new listen addresses */
        ...
        // TODO cwl let's parse "hash"
        if (strcmp(args[2], "hash") == 0)
            goto new_ver_cwl;
        // TODO cwl end let's parse "hash"
        ...
        Warning("add switching_rule be.name = %s\n", rule->be.name);
        LIST_INIT(&rule->list);
        LIST_ADDQ(&curproxy->switching_rules, &rule->list);
        // TODO cwl add new key word hash
        goto origin_cwl;
new_ver_cwl:
        if (*args[3]) {
            Warning("Add switching_hash_rule: %s -> %s\n", args[3], args[1]);
            union be *be_el = (union be*)calloc(1, sizeof(*be_el));
            memset(be_el, 0, sizeof(*be_el));
            be_el->name = strdup(args[1]);
            //hashtbl_insert(tbl_fun, args[3], be_el);
            hashtbl_insert(curproxy->switching_hashtbl, args[3], be_el);
            struct hashnode_s *node, *next;
            int n;
            //for (n = 0; n < tbl_fun->size; ++n) {
            //    for (node = tbl_fun->nodes[n]; node; node=next) {
            //        next = node->next;
            //        Warning("%s --> %p\n", node->key, node->data);
            //    }
            //}
            for (n = 0; n < curproxy->switching_hashtbl->size; ++n) {
                for (node = curproxy->switching_hashtbl->nodes[n]; node; node=next) {
                    Warning("%s --> %s\n", node->key, ((union be*)node->data)->name);
                    next = node->next;
                }
            }
        }
        else
            Warning("Missing domain name as hash key\n");

origin_cwl:
        Warning("origin point\n");
        // TODO cwl end add new key word hash
    }
    ...
    while (curproxy != NULL) {
        ...
        list_for_each_entry(rule, &curproxy->switching_rules, list) {
            ...
            if (!target) {
                ...
                Warning("backend(%p) found for backend_name(%s)(switching rule)\n", target, rule->be.name);
        ...
        // TODO cwl find target proxy for use_backend hash rules
        int n;
        struct hashnode_s *node, *next;
        for (n = 0; n < curproxy->switching_hashtbl->size; ++n) {
            for (node = curproxy->switching_hashtbl->nodes[n]; node; \
                    node = node->next) {
                struct proxy *target;
                union be *be_el = (union be*) node->data;
                target = findproxy_mode(be_el->name, curproxy->mode, PR_CAP_BE);
                if (!target) {
                    Alert("Proxy '%s': unable to find required use_backend hash: '%s'.\n", curproxy->id, be_el->name);
                    cfgerr++;
                } else if (target == curproxy) {
                    Alert("Proxy '%s': loop detected for use_backend: '%s'\n", curproxy->id, be_el->name);
                    cfgerr++;
                } else {
                    Warning("backend(%p) hashed: %s\n", target, be_el->name);
                    free((void*)be_el->name);
                    node->data = target;
                    target->bind_proc = curproxy->bind_proc ?
                        (target->bind_proc | curproxy->bind_proc) : 0;
                }
        ...
        // TODO cwl end find target proxy for use_backend hash rules
...
// TODO cwl gen a hash table for fun
__attribute__((constructor))
void __hash_table_init()
{
    if(!(tbl_fun = hashtbl_create(HASH_SIZE, NULL))) {
        fprintf(stderr, "ERROR: hashtbl_create() failed\n");
        exit(EXIT_FAILURE);
    }
    hashtbl_insert(tbl_fun, "TEST", "TEST_VALUE");
}
// TODO cwl end gen a hash table for fun
